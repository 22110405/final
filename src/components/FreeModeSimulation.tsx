...
interface FreeModeSimulationProps {
  ...
  audioRefs: {
    bubbling: MutableRefObject<HTMLAudioElement | null>;
    hissing: MutableRefObject<HTMLAudioElement | null>;
    explosion: MutableRefObject<HTMLAudioElement | null>;
    fizzing: MutableRefObject<HTMLAudioElement | null>;
    pouring: MutableRefObject<HTMLAudioElement | null>;
    smoke: MutableRefObject<HTMLAudioElement | null>;
    glass_break: MutableRefObject<HTMLAudioElement | null>;
    danger: MutableRefObject<HTMLAudioElement | null>;
  };
}
...
export const FreeModeSimulation = ({ 
  ...
  // تشغيل الأصوات بناءً على نتائج التفاعل
  useEffect(() => {
    if (soundEnabled && isReacting && reactionResult) {
      // تشغيل صوت الفوران إذا كانت هناك فقاعات
      if (reactionResult.hasBubbles && audioRefs.bubbling.current) {
        audioRefs.bubbling.current.play().catch(err => console.error('Error playing bubbling sound:', err));
      }
      // تشغيل صوت الهسهسة إذا كان هناك غاز
      if (reactionResult.hasGas && audioRefs.hissing.current) {
        audioRefs.hissing.current.play().catch(err => console.error('Error playing hissing sound:', err));
      }
      // تشغيل صوت الانفجار إذا كان هناك انفجار
      if (reactionResult.hasExplosion && audioRefs.explosion.current) {
        audioRefs.explosion.current.play().catch(err => console.error('Error playing explosion sound:', err));
      }
      // تشغيل صوت الفوران إذا كان هناك تفاعل خفيف
      if (reactionResult.hasPrecipitate && audioRefs.fizzing.current) {
        audioRefs.fizzing.current.play().catch(err => console.error('Error playing fizzing sound:', err));
      }
      // تشغيل صوت السكب أثناء إضافة مادة كيميائية
      if (isPouring && audioRefs.pouring.current) {
        audioRefs.pouring.current.play().catch(err => console.error('Error playing pouring sound:', err));
      }
      // تشغيل صوت الدخان إذا كان هناك دخان
      if (reactionResult.hasSmoke && audioRefs.smoke.current) {
        audioRefs.smoke.current.play().catch(err => console.error('Error playing smoke sound:', err));
      }
      // تشغيل صوت الكسر إذا انكسر أنبوب الاختبار
      if (isTestTubeBroken && audioRefs.glass_break.current) {
        audioRefs.glass_break.current.play().catch(err => console.error('Error playing glass break sound:', err));
      }
      // تشغيل صوت الخطر إذا كان التفاعل خطيرًا
      if ((reactionResult.hasFire || reactionResult.temperature > 100) && audioRefs.danger.current) {
        audioRefs.danger.current.play().catch(err => console.error('Error playing danger sound:', err));
      }
    }

    // إيقاف جميع الأصوات عند إيقاف التفاعل أو إلغاء التثبيت
    return () => {
  ...
  }, [isReacting, reactionResult, soundEnabled, isTestTubeBroken, isPouring, audioRefs]);
